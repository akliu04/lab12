...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0)
			 throw new TreeException("no colon detected");//a TreeException with a useful message
		String parent = line.substring(0,colonIndex);// The substring of line that starts at char #0 and ends just before colonIndex. Check the API for 
			// java.util.String, method substring(), if you need guidance.
		...
		else
		{
			 ...
			 if(root.getNodeWithName(parent)==null) throw new TreeException("error: parent node not found");
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("error: no node named" + name1);//??? Throw a TreeException with a useful message
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			throw new TreeException("error: no node named" + name2);
